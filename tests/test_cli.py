...
def test_cli_network_default():
    with tempfile.TemporaryDirectory() as tmp_dir:
        with patch('sonata_network_reduction.cli.reduce_network') as reduce_network_mock:
            ...
            runner = CliRunner()
            result = runner.invoke(network, [str(circuit_config_file), tmp_dir])
            assert reduce_network_mock.call_count == 1
            assert result.exit_code == 0
            args = reduce_network_mock.call_args[0]
            assert args == (Path(circuit_config_file), Path(tmp_dir))
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        with patch('sonata_network_reduction.cli.reduce_network') as reduce_network_mock:
            ...
            result = runner.invoke(
                network, [str(circuit_config_file), tmp_dir, '--reduction_frequency', '5'])
            assert reduce_network_mock.call_count == 1
            assert result.exit_code == 0
            args = reduce_network_mock.call_args[0]
            assert args == (Path(circuit_config_file), Path(tmp_dir))
...
def test_cli_node():
    with tempfile.TemporaryDirectory() as tmp_dir:
        with patch('sonata_network_reduction.cli._reduce_node_same_process') as reduce_node_mock:
            ...
            runner = CliRunner()
            result = runner.invoke(node, ['0', 'cortex', str(circuit_config_file), tmp_dir])
            assert reduce_node_mock.call_count == 1
            assert result.exit_code == 0
            args = reduce_node_mock.call_args[0]
            assert args == (0, 'cortex', Path(circuit_config_file), Path(tmp_dir))
